...
list (``--selected``, one ``catchment_name`` per row in CSV). The bundled
``selected_catchments_1347.csv`` carries the 1,347-catchment subset used in
the released results.

CAMELS-DE 1.0 is available on Zenodo (see ``download_camels_de.sh``).
"""
import argparse
import pathlib
import pickle
...
def main():
    ...
    # Also dump the LSTM-format long CSV (catchment_name, Date, P, T, PET, Q)
    # restricted to the test window 2010-01-01 .. 2020-12-31 (first 365 d are
    # warm-up, scoring is 2011-2020). The LSTM forward in reproduce.py reads it.
    test_csv = args.out_dir / "data_test_CAMELS_DE1.00.csv"
    print(f"writing LSTM long-form CSV -> {test_csv} ...")
    test_mask = (ALL_TIME >= "2010-01-01") & (ALL_TIME <= "2020-12-31")
    t_idx = np.where(test_mask)[0]
    n_catch = forcings.shape[0]; n_t = len(t_idx)
    rows = {
        "catchment_name": np.repeat(np.asarray(ids), n_t),
        "Date": np.tile(ALL_TIME[test_mask].strftime("%Y-%m-%d"), n_catch),
        "P": forcings[:, t_idx, 0].reshape(-1),
        "T": forcings[:, t_idx, 1].reshape(-1),
        "PET": forcings[:, t_idx, 2].reshape(-1),
        "Q": target[:, t_idx, 0].reshape(-1),
    }
    pd.DataFrame(rows).to_csv(test_csv, index=False)
    print(f"wrote {test_csv} ({n_t} days × {n_catch} catchments)")
